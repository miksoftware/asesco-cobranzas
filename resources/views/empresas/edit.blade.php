...
@section('title', 'Editar — ' . $empresa->nombre)
...
<div class="max-w-5xl mx-auto">
    ...
    <div class="flex items-center justify-between mb-6">
        ...
        <a href="{{ route('empresas.show', $empresa) }}"
           class="inline-flex items-center gap-2 text-sm font-medium text-gray-600 border border-gray-200 px-4 py-2 rounded-lg hover:bg-gray-50 transition-colors">
    ...
    {{-- Info banner --}}
    <div class="bg-blue-50 border border-blue-100 rounded-xl px-4 py-3 mb-5 flex items-center gap-3 text-sm text-blue-700">
        ...
        Puedes hacer clic en cualquier paso para editarlo directamente, o usar "Guardar y salir" para guardar un paso y volver al listado.
    </div>

    {{-- Steps indicator (todos clickeables en edición) --}}
    <div class="bg-white rounded-xl border border-gray-200 p-5 mb-6">
        <div class="flex items-center justify-between relative">
            ...
            <button type="button" onclick="goStep({{ $idx + 1 }})"
                    class="flex flex-col items-center z-10 step-item cursor-pointer" data-step="{{ $idx + 1 }}"
    ...
    <div id="step-1" class="step-panel">
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <div class="mb-6">
                ...
                <p class="text-sm text-gray-500 mt-1">Edita el nombre de la empresa.</p>
            </div>
            <div class="max-w-md">
                <label class="block text-sm font-semibold text-gray-700 mb-2">Nombre de la empresa <span class="text-red-500">*</span></label>
                <input type="text" id="empresa-nombre" value="{{ $empresa->nombre }}" placeholder="Ej: SEGUROS S.A."
            ...
            <div class="flex items-center justify-between mt-8">
                <a href="{{ route('empresas.index') }}" class="inline-flex items-center gap-2 text-sm font-medium text-gray-600 bg-gray-100 px-5 py-2.5 rounded-lg hover:bg-gray-200 transition-colors">
                    Cancelar
                </a>
                <div class="flex gap-3">
                    <button onclick="guardarNombre(true)" class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 border border-gray-300 px-5 py-2.5 rounded-lg hover:bg-gray-50 transition-colors">
                    ...
                    <button onclick="guardarNombre(false)" id="btn-step1"
    ...
    <div id="step-2" class="step-panel hidden">
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <button onclick="agregarTramoTarifa()" class="inline-flex items-center gap-2 text-sm font-semibold text-white bg-gradient-to-r from-[#E8611A] to-[#C94477] px-4 py-2 rounded-lg hover:opacity-90 transition-opacity">
            ...
            <div class="flex items-center justify-between mt-8">
                ...
                <div class="flex gap-3">
                    <button onclick="guardarTarifas(true)" class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 border border-gray-300 px-5 py-2.5 rounded-lg hover:bg-gray-50 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"/></svg>
                        Guardar y salir
                    </button>
                    <button onclick="guardarTarifas(false)" class="inline-flex items-center gap-2 bg-gradient-to-r from-[#E8611A] to-[#C94477] text-white text-sm font-semibold px-6 py-2.5 rounded-lg hover:opacity-90 transition-opacity">
                        Guardar y Continuar
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                    </button>
                </div>
    ...
    <div id="step-3" class="step-panel hidden">
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <button onclick="agregarCanal()" class="inline-flex items-center gap-2 text-sm font-semibold text-white bg-gradient-to-r from-[#E8611A] to-[#C94477] px-4 py-2 rounded-lg hover:opacity-90 transition-opacity">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                    Agregar Canal
                </button>
            </div>
            ...
            <div class="flex items-center justify-between mt-8">
                ...
                <div class="flex gap-3">
                    <button onclick="guardarCanales(true)" class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 border border-gray-300 px-5 py-2.5 rounded-lg hover:bg-gray-50 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"/></svg>
                        Guardar y salir
                    </button>
                    <button onclick="guardarCanales(false)" class="inline-flex items-center gap-2 bg-gradient-to-r from-[#E8611A] to-[#C94477] text-white text-sm font-semibold px-6 py-2.5 rounded-lg hover:opacity-90 transition-opacity">
                        Guardar y Continuar
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M9 5l7 7-7 7"/></svg>
                    </button>
                </div>
        ...
        <div class="bg-white rounded-xl border border-gray-200 p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <div class="flex gap-2">
                    <button onclick="agregarLineamiento('porcentaje')" class="inline-flex items-center gap-1.5 text-sm font-semibold text-white bg-[#E8611A] px-4 py-2 rounded-lg hover:opacity-90 transition-opacity">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                        Por Porcentaje
                    </button>
                    <button onclick="agregarLineamiento('tramo')" class="inline-flex items-center gap-1.5 text-sm font-semibold text-white bg-[#C94477] px-4 py-2 rounded-lg hover:opacity-90 transition-opacity">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg>
                        Por Tramo
                    </button>
                </div>
            ...
            <div class="flex items-center justify-between mt-8">
                ...
                <div class="flex gap-3">
                    <button onclick="guardarLineamientos(true)" class="inline-flex items-center gap-2 text-sm font-medium text-gray-700 border border-gray-300 px-5 py-2.5 rounded-lg hover:bg-gray-50 transition-colors">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M8 7H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2h-3m-1 4l-3 3m0 0l-3-3m3 3V4"/></svg>
                        Guardar y salir
                    </button>
                    <button onclick="guardarLineamientos(false)" class="inline-flex items-center gap-2 bg-gradient-to-r from-[#E8611A] to-[#C94477] text-white text-sm font-semibold px-6 py-2.5 rounded-lg hover:opacity-90 transition-opacity">
...
// Datos existentes
const existingTarifas = @json($empresa->tarifas);
const existingCanales = @json($empresa->canales);
const existingLineamientos = @json($empresa->lineamientos->load('porcentaje', 'tramos'));
...
function salirAlListado() { window.location.href = '/empresas'; }
...
async function guardarNombre(salir = false) {
    ...
    try {
        await apiCall('PATCH', `/empresas/${empresaId}/nombre`, { nombre });
...
function agregarTramoTarifa(data = {}) {
    ...
    tr.innerHTML = `
        ...
        <td class="px-4 py-2"><input type="text" placeholder="Ej: 1 a 30" value="${escHtml(data.nombre_tramo||'')}" class="tarifa-nombre w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-[#E8611A]"></td>
        <td class="px-4 py-2 bg-orange-50"><div class="flex items-center gap-1"><input type="number" min="0" max="100" step="0.01" placeholder="0" value="${data.porcentaje_vigente??''}" class="tarifa-vigente w-20 border border-gray-200 rounded-lg px-2 py-1.5 text-sm text-center focus:outline-none focus:border-[#E8611A]"><span class="text-gray-400 text-xs">%</span></div></td>
        <td class="px-4 py-2 bg-pink-50"><div class="flex items-center gap-1"><input type="number" min="0" max="100" step="0.01" placeholder="0" value="${data.porcentaje_castigada??''}" class="tarifa-castigada w-20 border border-gray-200 rounded-lg px-2 py-1.5 text-sm text-center focus:outline-none focus:border-[#C94477]"><span class="text-gray-400 text-xs">%</span></div></td>
        <td class="px-4 py-2 text-center"><button onclick="document.getElementById('tarifa-row-${n}').remove(); renumerarFilas('tarifas-tbody')" class="w-7 h-7 flex items-center justify-center text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg mx-auto"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button></td>`;
...
async function guardarTarifas(salir = false) {
    ...
    rows.forEach(r => {
        ...
        tarifas.push({ nombre_tramo: nombre || '', porcentaje_vigente: r.querySelector('.tarifa-vigente')?.value || 0, porcentaje_castigada: r.querySelector('.tarifa-castigada')?.value || 0 });
    });
    ...
    try {
        await apiCall('POST', `/empresas/${empresaId}/tarifas`, { tarifas });
        if (salir) { salirAlListado(); return; }
        goStep(3);
...
function agregarCanal(data = {}) {
    ...
    const n = canalCount;
    const idx = tbody.children.length + 1;
    const opts = mediosPago.map(m => `<option value="${m}" ${data.medio_pago === m ? 'selected' : ''}>${m}</option>`).join('');
    const tr = document.createElement('tr');
    tr.className = 'border-t border-gray-100 hover:bg-gray-50 transition-colors';
    tr.id = `canal-row-${n}`;
    tr.innerHTML = `
        <td class="px-4 py-2 text-gray-400 text-xs">${idx}</td>
        <td class="px-4 py-2"><input type="text" placeholder="Ej: BANCOLOMBIA" value="${escHtml(data.nombre_canal||'')}" class="canal-nombre w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-[#E8611A]"></td>
        <td class="px-4 py-2"><input type="text" placeholder="Ej: 1234567890" value="${escHtml(data.numero_canal||'')}" class="canal-numero w-36 border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-[#E8611A]"></td>
        <td class="px-4 py-2"><select class="canal-medio w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-[#E8611A]"><option value="">-- Seleccionar --</option>${opts}</select></td>
        <td class="px-4 py-2 text-center"><button onclick="document.getElementById('canal-row-${n}').remove(); renumerarFilas('canales-tbody')" class="w-7 h-7 flex items-center justify-center text-red-400 hover:text-red-600 hover:bg-red-50 rounded-lg mx-auto"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button></td>`;
    tbody.appendChild(tr);
}
...
async function guardarCanales(salir = false) {
    ...
    try {
        await apiCall('POST', `/empresas/${empresaId}/canales`, { canales });
        if (salir) { salirAlListado(); return; }
        goStep(4);
...
function agregarLineamiento(tipo, data = {}) {
    ...
    const isActivo = data.hasOwnProperty('is_active') ? !!data.is_active : (container.children.length === 0);
    div.className = 'border-2 ' + (isActivo ? 'border-[#E8611A]' : 'border-gray-200') + ' rounded-xl p-5 transition-all duration-200';
    div.id = `lin-${n}`;
    ...
    if (tipo === 'porcentaje') {
        ...
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3 flex-1">
                <label class="flex items-center gap-2 cursor-pointer" title="Marcar como lineamiento activo">
                    <input type="radio" name="lin-activo" value="${n}" class="lin-radio accent-[#E8611A]" ${isActivo ? 'checked' : ''} onchange="marcarActivo(${n})">
                    <span class="text-xs font-semibold px-2.5 py-1 rounded-full ${colorBadge}">${labelTipo}</span>
                </label>
                <input type="text" placeholder="Concepto (Ej: CAPITAL)" value="${escHtml(data.concepto||'')}" class="lin-concepto border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-[#E8611A] w-64">
            </div>
            <button onclick="document.getElementById('lin-${n}').remove()" class="text-red-400 hover:text-red-600 ml-4"><svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg></button>
        </div>
        ...
        <div class="grid grid-cols-2 gap-4 max-w-sm">
            <div>
                ...
                <div class="flex items-center gap-1">
                    <input type="number" min="0" max="100" step="0.01" placeholder="0" value="${data.porcentaje?.porcentaje_vigente??''}" class="lin-pct-vigente w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-center focus:outline-none focus:border-[#E8611A]">
            ...
            <div>
                ...
                <div class="flex items-center gap-1">
                    <input type="number" min="0" max="100" step="0.01" placeholder="0" value="${data.porcentaje?.porcentaje_castigado??''}" class="lin-pct-castigado w-full border border-gray-200 rounded-lg px-3 py-1.5 text-sm text-center focus:outline-none focus:border-[#C94477]">
                    <span class="text-gray-400 text-xs">%</span>
                </div>
            </div>
        </div>`;
        ...
        <div class="flex items-center justify-between mb-4">
            <div class="flex items-center gap-3 flex-1">
                <label class="flex items-center gap-2 cursor-pointer" title="Marcar como lineamiento activo">
                    <input type="radio" name="lin-activo" value="${n}" class="lin-radio accent-[#C94477]" ${isActivo ? 'checked' : ''} onchange="marcarActivo(${n})">
                    <span class="text-xs font-semibold px-2.5 py-1 rounded-full ${colorBadge}">${labelTipo}</span>
                </label>
                <input type="text" placeholder="Concepto (Ej: CAPITAL)" value="${escHtml(data.concepto||'')}" class="lin-concepto border border-gray-200 rounded-lg px-3 py-1.5 text-sm focus:outline-none focus:border-[#E8611A] w-64">
            </div>
        ...
        <div class="grid grid-cols-1 xl:grid-cols-2 gap-6">
            <div>
                <div class="flex items-center justify-between mb-2">
                    ...
                    <button onclick="agregarTramoLineamiento('tramos-vigente-${n}','vigente')" class="text-xs text-[#E8611A] hover:underline flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg> Agregar</button>
                </div>
                <div class="border border-orange-100 rounded-lg overflow-hidden">
                    <table class="w-full text-xs"><thead class="bg-orange-50"><tr><th class="text-left px-3 py-2 font-semibold text-[#E8611A]">Tramo</th><th class="text-center px-3 py-2 font-semibold text-[#E8611A]">% Vigente</th><th class="w-8"></th></tr></thead><tbody id="tramos-vigente-${n}"></tbody></table>
                </div>
            </div>
            <div>
                <div class="flex items-center justify-between mb-2">
                    <h4 class="text-xs font-bold text-[#C94477] uppercase tracking-wide">Tramos Castigado</h4>
                    <button onclick="agregarTramoLineamiento('tramos-castigado-${n}','castigado')" class="text-xs text-[#C94477] hover:underline flex items-center gap-1"><svg class="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M12 4v16m8-8H4"/></svg> Agregar</button>
                </div>
                <div class="border border-pink-100 rounded-lg overflow-hidden">
                    <table class="w-full text-xs"><thead class="bg-pink-50"><tr><th class="text-left px-3 py-2 font-semibold text-[#C94477]">Tramo</th><th class="text-center px-3 py-2 font-semibold text-[#C94477]">% Castigado</th><th class="w-8"></th></tr></thead><tbody id="tramos-castigado-${n}"></tbody></table>
                </div>
            </div>
        </div>`;
    }
...
function agregarTramoLineamiento(tbodyId, tipo, data = {}) {
    ...
    tr.innerHTML = `
        <td class="px-3 py-1.5"><input type="text" placeholder="Ej: 1 a 30" value="${escHtml(data.nombre_tramo||'')}" class="tl-nombre w-full border border-gray-200 rounded px-2 py-1 text-xs focus:outline-none"></td>
        <td class="px-3 py-1.5"><div class="flex items-center gap-1 justify-center"><input type="number" min="0" max="100" step="0.01" placeholder="0" value="${data.porcentaje||''}" class="tl-pct w-16 border border-gray-200 rounded px-2 py-1 text-xs text-center focus:outline-none"><span class="text-gray-400">%</span></div></td>
        <td class="px-2 py-1.5 text-center"><button onclick="document.getElementById('${rowId}').remove()" class="text-red-400 hover:text-red-600"><svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24" stroke-width="2"><path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12"/></svg></button></td>`;
    tbody.appendChild(tr);
}
...
async function guardarLineamientos(salir = false) {
    ...
    try {
        await apiCall('POST', `/empresas/${empresaId}/lineamientos`, { lineamientos });
        if (salir) { salirAlListado(); return; }
        await Swal.fire({ title: '¡Cambios guardados!', text: 'La configuración de la empresa fue actualizada.', icon: 'success', confirmButtonColor: '#E8611A', confirmButtonText: 'Ver empresas' });
...
document.addEventListener('DOMContentLoaded', () => {
    goStep(1);
    existingTarifas.forEach(t => agregarTramoTarifa(t));
    existingCanales.forEach(c => agregarCanal(c));
    existingLineamientos.forEach(lin => agregarLineamiento(lin.tipo, lin));
});
</script>
@endpush
