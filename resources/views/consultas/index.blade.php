...
<div x-data="consultaPage()" class="space-y-5">
                                        ...
                                        <div class="flex items-center justify-center gap-1 relative">
                                            ...
                                            <template x-if="pago.soporte">
                                                <div class="flex items-center gap-1">
                                                    <button type="button" @click="openSoporteModal(pago.soporte)" class="text-green-600 hover:bg-green-50 p-1.5 rounded transition-colors cursor-pointer" title="Ver soporte">
                                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"></path><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"></path></svg>
                                                    </button>
                                                    <button type="button" @click="downloadSoporte(getSoporteUrl(pago.soporte), getSoporteFileName(pago.soporte))" class="text-blue-600 hover:bg-blue-50 p-1.5 rounded transition-colors cursor-pointer" title="Descargar soporte">
                                                    ...
                                                    <div class="flex items-center justify-center gap-2 relative">
                                                        <template x-if="!a.isNew">
                                                            <div class="flex gap-2">
                                                                <button type="button" @click="openSoporteModal(a.archivo_path)" class="p-1 rounded bg-blue-50 text-blue-600 hover:bg-blue-100 transition-colors cursor-pointer" title="Ver Documento">
                                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z"/><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M2.458 12C3.732 7.943 7.523 5 12 5c4.478 0 8.268 2.943 9.542 7-1.274 4.057-5.064 7-9.542 7-4.477 0-8.268-2.943-9.542-7z"/></svg>
                                                                </button>
                                                                <button type="button" @click="downloadSoporte(getSoporteUrl(a.archivo_path), getSoporteFileName(a.archivo_path))" class="p-1 rounded bg-green-50 text-green-600 hover:bg-green-100 transition-colors cursor-pointer" title="Descargar Documento">
                                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"></path></svg>
                                                                </button>
    ...
    <template x-teleport="body">
        <div x-show="modal"
            ...
            <div x-show="modal"
                 ...
                 style="width:100%;max-width:760px;max-height:85vh;display:flex;flex-direction:column;border-radius:14px;overflow:hidden;box-shadow:0 25px 60px rgba(0,0,0,0.35);">
                {{-- Dark header --}}
                <div style="background:#1e2532;padding:16px 20px 14px;flex-shrink:0;display:flex;align-items:flex-start;justify-content:space-between;gap:12px;">
                    <div style="min-width:0;">
                        <div style="display:flex;align-items:center;gap:6px;margin-bottom:3px;">
                            <svg style="width:12px;height:12px;flex-shrink:0;" fill="none" stroke="#f97316" viewBox="0 0 24 24" stroke-width="2">
                                <path stroke-linecap="round" stroke-linejoin="round" d="M15 9h3.75M15 12h3.75M15 15h3.75M4.5 19.5h15a2.25 2.25 0 002.25-2.25V6.75A2.25 2.25 0 0019.5 4.5h-15a2.25 2.25 0 00-2.25 2.25v10.5A2.25 2.25 0 004.5 19.5zm6-10.125a1.875 1.875 0 11-3.75 0 1.875 1.875 0 013.75 0zm1.294 6.336a6.721 6.721 0 01-3.17.789 6.721 6.721 0 01-3.168-.789 3.376 3.376 0 016.338 0z"/>
                            </svg>
                            <p style="font-size:11px;color:#f97316;text-transform:uppercase;letter-spacing:.08em;font-weight:700;" x-text="modalRecord ? modalRecord._systemName : ''"></p>
                        </div>
                        <p style="font-size:14px;font-weight:600;color:#f1f5f9;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;" x-text="modalRecord ? (getNombres(modalRecord) + ' ' + getApellidos(modalRecord)).trim() : ''"></p>
                        <p style="font-size:11px;color:#64748b;margin-top:2px;font-family:monospace;" x-text="modalRecord ? getCedulaField(modalRecord) : ''"></p>
                    </div>
                    <button @click="modal = false" style="flex-shrink:0;width:28px;height:28px;border-radius:7px;border:none;background:rgba(255,255,255,0.08);color:#94a3b8;display:flex;align-items:center;justify-content:center;cursor:pointer;" onmouseover="this.style.background='rgba(255,255,255,0.15)'" onmouseout="this.style.background='rgba(255,255,255,0.08)'">
                ...
                {{-- Body --}}
                <div style="flex:1;overflow-y:auto;background:#ffffff;padding:16px 20px;">
                    <template x-if="modalRecord">
                        <div>
                            {{-- Flat attributes (2 columns) --}}
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-0.5">
                                <template x-for="(value, key) in modalRecord" :key="key">
                                    <template x-if="!String(key).startsWith('_') && key !== 'status' && key !== 'error_message' && !isObject(value) && !isArray(value) && hasValue(value)">
                                        <div style="display:flex;align-items:baseline;justify-content:space-between;gap:8px;padding:7px 0;border-bottom:1px solid #f1f5f9;">
                                            <p style="font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:#94a3b8;flex-shrink:0;max-width:45%;line-height:1.3;" x-text="formatLabel(key)"></p>
                                            <p style="font-size:11.5px;color:#1e293b;font-weight:500;text-align:right;word-break:break-all;" x-text="formatValue(value)"></p>
                                        </div>
                                    </template>
                                </template>
                            </div>

                            {{-- Sub-objects / Sections --}}
                            <template x-for="(value, key) in modalRecord" :key="'s-' + key">
                                <template x-if="isObject(value)">
                                    <div style="margin-top:16px;">
                                        <div style="padding:6px 0;border-bottom:2px solid #e2e8f0;margin-bottom:8px;">
                                            <p style="font-size:10.5px;font-weight:700;color:#475569;text-transform:uppercase;letter-spacing:.08em;" x-text="formatLabel(key)"></p>
                                        </div>
                                        <div class="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-0.5">
                                            <template x-for="(subVal, subKey) in value" :key="subKey">
                                                <template x-if="hasValue(subVal) && subKey !== 'status' && subKey !== 'error_message'">
                                                    <div style="display:flex;align-items:baseline;justify-content:space-between;gap:8px;padding:7px 0;border-bottom:1px solid #f1f5f9;">
                                                        <p style="font-size:10px;text-transform:uppercase;letter-spacing:.07em;color:#94a3b8;flex-shrink:0;max-width:45%;line-height:1.3;" x-text="formatLabel(subKey)"></p>
                                                        <p style="font-size:11.5px;color:#1e293b;font-weight:500;text-align:right;word-break:break-all;" x-text="formatValue(subVal)"></p>
                                                    </div>
                                                </template>
                                            </template>
                                        </div>
    ...
    <div x-show="soporteModalOpen" 
         ...
         style="display: none;" 
         class="fixed inset-0 z-[10000] flex items-center justify-center bg-gray-900/70 backdrop-blur-sm p-4"
